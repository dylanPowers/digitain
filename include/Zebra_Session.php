 ...
 */
/* 
	Edited by Dylan Powers 7/2/2011.
	Changes:
		-Modified to work with an OO mysqli connection.
*/
class Zebra_Session
{
     ...
     *  @return void
     */
    function Zebra_Session($mysqli_connection, $session_lifetime = '', $gc_probability = '', $gc_divisor = '', $security_code = 'i2y10T4', $table_name = 'session_data', $lock_timeout = 60)
    {
        // continue if there is an active MySQL connection
        if (@$mysqli_connection->ping()) 
		{
			...
			// Set the mySQLi database connection
        ...
        // if no MySQL connections could be found
            ...
            trigger_error('<br>No MySQLi connection!<br>Error', E_USER_ERROR);
    ...
    function close()
    {
        ...
        $this->mysqli_connection->query('SELECT RELEASE_LOCK("' . $this->session_lock . '")')

            // stop execution and print message on error
            or die($this->mysqli_connection->error);
    ...
    function destroy($session_id)
    {
        ...
        $result = $this->mysqli_connection->query('

            DELETE FROM
                ' . $this->table_name . '
            WHERE
                session_id = "' . $this->mysqli_connection->real_escape_string($session_id) . '"

        ') or die($this->mysqli_connection->error);

        // if anything happened
        if ($this->mysqli_connection->affected_rows !== -1) {
    ...
    function gc($maxlifetime)
    {
        ...
        $result = $this->mysqli_connection->query('
            ...
            DELETE FROM
                ' . $this->table_name . '
            WHERE
                session_expire < "' . $this->mysqli_connection->real_escape_string(time() - $maxlifetime) . '"
    ...
    function get_active_sessions()
    {
        ...
        $result = @$this->mysqli_connection->fetch_assoc($this->mysqli_connection->query('
    ...
    function read($session_id)
    {
        ...
        $this->session_lock = $this->mysqli_connection->real_escape_string('session_' . $session_id);

        // try to obtain a lock with the given name and timeout
        $result = $this->mysqli_connection->query('SELECT GET_LOCK("' . $this->session_lock . '", ' . $this->mysqli_connection->real_escape_string($this->lock_timeout) . ')');
        ...
        $result = $this->mysqli_connection->query('
            ...
            FROM
                ' . $this->table_name . '
            WHERE
                session_id = "' . $this->mysqli_connection->real_escape_string($session_id) . '" AND
                session_expire > "' . time() . '" AND
                http_user_agent = "' . $this->mysqli_connection->real_escape_string(md5((isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '') . $this->security_code)) . '"
            LIMIT 1
    ...
    function write($session_id, $session_data)
    {
		...
		$result = $this->mysqli_connection->query('
            ...
            VALUES (
                "' . $this->mysqli_connection->real_escape_string($session_id) . '",
                "' . $this->mysqli_connection->real_escape_string(md5((isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '') . $this->security_code)) . '",
                "' . $this->mysqli_connection->real_escape_string($session_data) . '",
                "' . $this->mysqli_connection->real_escape_string(time() + $this->session_lifetime) . '"
            )
            ON DUPLICATE KEY UPDATE
                session_data = "' . $this->mysqli_connection->real_escape_string($session_data) . '",
                session_expire = "' . $this->mysqli_connection->real_escape_string(time() + $this->session_lifetime) . '"
