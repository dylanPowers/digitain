...
//*****Check if the user is a valid Digitain.com bonus content user, and second check that
//it isn't our own torrent server trying to access the tracker.

$query = @mysql_query('Select COUNT(*) from digitain.users where last_ip='.mysql_real_escape_string($ip).';');
$valid_ips = mysql_result($query, 0);
if ($valid_ips <= 0){
    if (ip2dec(gethostbyname('digi-tain.com')) !=$ip){
        errorexit("Invalid ip address, please log in to the website");
    }
}
